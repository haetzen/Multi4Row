...
class Player:
	def __init__(self, id, startpos, startpos_x_grid, color, gridsize, cellsize, offset_grid, screen_size, funks, map, map_dimensions, client):
		...
		self.cursor = pygame.Surface((self.cellsize, self.offset_grid - startpos[1] - (self.cellsize/2) + self.gridsize), pygame.SRCALPHA)
		self.cursor.fill((0, 0, 0,))
		self.cursor.set_colorkey((0, 0, 0))
		self.cursor.fill((self.color[0], self.color[1], self.color[2], 25))
	...
	def render(self, screen, dt, target_fps):
		if self.dropping:
			...
			if self.y + (self.velocity * dt * target_fps) < self.target[1] * self.cellsize + self.offset_grid:
				...
				self.cursor = pygame.Surface((self.cellsize, self.offset_grid + self.gridsize - self.y - (self.cellsize/2)), pygame.SRCALPHA)
				self.cursor.fill((0, 0, 0,))
				self.cursor.set_colorkey((0, 0, 0))
				self.cursor.fill((self.color[0], self.color[1], self.color[2], 25))	
		...
		self.FUNKS.outline(surf, (self.x, self.y), screen, 4, self.lock_fall and self.color or (0, 255, 0))
		screen.blit(surf, (self.x, self.y))
		screen.blit(self.cursor, (self.x, self.y + (self.cellsize/2)))
